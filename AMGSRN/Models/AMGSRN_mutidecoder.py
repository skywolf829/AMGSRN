...
class AMGSRN_mutidecoder(nn.Module):
    def __init__(self, opt):
        super(AMGSRN_mutidecoder, self).__init__()
        ...
        if(opt['use_tcnn_if_available']):
            try:
                self.decoders = torch.nn.ModuleList([init_decoder_tcnn() for _ in range(2)])
            except ImportError:
                print(f"Tried to use TinyCUDANN but found it was not installed - reverting to PyTorch layers.")
                self.decoders = torch.nn.ModuleList([init_decoder_pytorch() for _ in range(2)])
        else:                
            self.decoders = torch.nn.ModuleList([init_decoder_pytorch() for _ in range(2)])
    ...
    def get_model_parameters(self):
        return [
            ...
            *[{"params": decoder.parameters()} for decoder in self.decoders]
        ]
    ...
    def forward(self, x : torch.Tensor) -> torch.Tensor:
        ...
        y = self.decoders[0](feats)
        y += self.decoders[1](feats) * 1e-4
