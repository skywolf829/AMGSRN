...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, opt):
        ...
        if os.path.isdir(os.path.join(data_folder, self.opt['data'])):
            ...
            # remove anything that isn't a .nc file
            self.file_names = [os.path.join(data_folder, self.opt['data'], f) for f in self.file_names if f.endswith('.nc')]
    ...
    def min(self):
        if self.min_ is not None:
            ...
            for t in range(len(self.file_names)):
                ...
                self.min_ = min(self.min_, self.data[t].min().item())
    ...
    def mean(self):
        if self.mean_ is not None:
            ...
            for t in range(len(self.file_names)):
                self.load_timestep(t)
                self.mean_ += self.data[t].mean().item()
                self.unload_timestep(t)
    ...
    def max(self):
        if self.max_ is not None:
            ...
            for t in range(len(self.file_names)):
                self.load_timestep(t)
                self.max_ = max(self.max_, self.data[t].max().item())
                self.unload_timestep(t)
            return self.max_
